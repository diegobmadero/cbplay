...
async def stream_tts_chunks(
    combined_texts, voice, model, instructions, response_format="pcm", timeout=240
):
    if not AsyncOpenAI or not LocalAudioPlayer:
        raise ImportError(
            "AsyncOpenAI/LocalAudioPlayer not available in installed openai package."
        )

    if response_format.lower() != "pcm":
        debug_print(
            "Streaming overrides response_format to 'pcm' for LocalAudioPlayer playback."
        )
        response_format = "pcm"
...
def run_interactive_tts_flow(
    clipboard_content, tts_texts, display_texts, args, model, provider
):
    ...
    for i, (tts_chunk, disp_chunk) in enumerate(zip(tts_texts, display_texts)):
        ...
        if "[Diagram omitted]" in tts_chunk or "[Table omitted]" in tts_chunk:
            ...
            debug_log_file(
                f"[MAIN] skip chunk {i}: has diagram/table omitted placeholder"
            )
    ...
    debug_log_file(
        f"[MAIN] total chunks={len(tts_texts)}, skip_highlight_chunks={skip_highlight_chunks}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="cbplay - Clipboard Play: auto-detect clipboard/file and transcribe or speak"
    )
    ...
    parser.add_argument(
        "-a", "--audio-file", help="Audio file to transcribe in stt mode"
    )
    ...
    parser.add_argument(
        "--model", default=None, help="TTS model (default: provider-specific)"
    )

    parser.add_argument(
        "--highlight-tts-model", default=None, help="TTS model when --highlight enabled"
    )
    ...
    parser.add_argument(
        "--chunk-size", type=int, default=300, help="Max chars per chunk (default: 300)"
    )
    ...
    parser.add_argument("--debug", action="store_true", help="Show debug info in UI")

    parser.add_argument("--debug-file", default=None, help="Debug log file path")
...
def main():
    ...
    if args.voice is None:
        args.voice = "verse" if args.provider == "openai" else "Leda"
    ...
    if args.stream and args.highlight:
        if "--highlight" in sys.argv:
            ...
            return
        print(
            "Note: --highlight disabled because --stream uses non-interactive playback."
        )
    ...
    if effective_mode == "stt":
        if not audio_path:
            print(
                "No valid audio file found. Provide --audio-file or copy path to clipboard."
            )
            return
    ...
    if not args.stream:
        ...
        run_interactive_tts_flow(
            clipboard_content, tts_texts, display_texts, args, tts_model, args.provider
        )
        return
    ...
    try:
        ...
        run_interactive_tts_flow(
            clipboard_content, tts_texts, display_texts, args, args.model, args.provider
        )
